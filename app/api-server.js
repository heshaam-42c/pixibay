...
api.delete('/api/picture/:id', api_token_check, function (req, res) {
	...
	pictures.findOne({ _id: req.params.id }, function (err, result) {
		if (err) {
			console.log('>>> Query error...' + err);
			res.status(500).json({ "message": "system error" });
		}
		...
		} else if (result.creator_id != req.user.user_profile._id) {
			console.log('>>> Unauthorized user trying to delete picture');
